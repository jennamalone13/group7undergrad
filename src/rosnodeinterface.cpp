...
#define ROS_SET_RTM_ERROR_STATUS(status) rtmSetErrorStatus(ros_controller1_dm_M,(status));
#define ROS_GET_RTM_ERROR_STATUS()     rtmGetErrorStatus(ros_controller1_dm_M)
#define ROS_RTM_STEP_TASK(id)          rtmStepTask(ros_controller1_dm_M,id)
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"ros_controller1_dm\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        ROS_SET_RTM_ERROR_STATUS(NULL);
        ros_controller1_dm_initialize();
        ...
        mSchedulerTimer = std::make_shared<ros::WallTimer>
          (mNode->createWallTimer(ros::WallDuration(1000000000*1e-9),
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(ROS_GET_RTM_ERROR_STATUS()
                == (NULL)) || rtmGetStopRequested(ros_controller1_dm_M));
